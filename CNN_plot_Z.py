...
# ======= Projection for plot  ========
data = nc.Dataset('data/z10_lag5_to_lead9_waccm6_exp1_regrid_bilin.nc','r')
...
# 2D U data
Z_SSW = data.variables['z_lag0'][:]  # shape:780*36*36 (EXP2)
Z_nSSW = data.variables['z_clim'][:]
Zmean_SSW = np.mean(Z_SSW, axis=0)   # composite of all EXP2 cases
Zmean_nSSW = np.mean(Z_nSSW, axis=0)
Zmean_SSW = Zmean_SSW.flatten()
Zmean_nSSW = Zmean_nSSW.flatten()
# Directory of classification results
resDir = 'models/validation_result_Z/'

# ======= Plot =======
levels = np.linspace(29000,31000,11)
...
for i in range(len(zeros_len)):
    ...
    im1 = ax[i].tripcolor(x,y, error_sum_SSWm[mask], vmax=100, vmin=0, cmap='Greys', alpha=.6)
    ax[i].coastlines(resolution='110m', color='dimgrey', lw=.8)
    ax[i].tricontourf(x,y, Zmean_SSW[mask], levels=levels, cmap='seismic', alpha=.15)
    ct = ax[i].tricontour(x,y, Zmean_SSW[mask], levels=levels, cmap='seismic',linewidths=1.5)
    gl = ax[i].gridlines(draw_labels=False, color='mediumblue', alpha=.8)
...
plt.tight_layout()
plt.savefig('figures/CNN_SSW_errorplot_Z.png', dpi=250)
...
for i in range(len(zeros_len)):
    ...
    im1 = ax[i].tripcolor(x,y, error_sum_nSSWm[mask], vmax=100, vmin=0, cmap='Greys', alpha=.6)
    ax[i].coastlines(resolution='110m', color='dimgrey', lw=.8)
    ax[i].tricontourf(x,y, Zmean_nSSW[mask], levels=levels, cmap='seismic', alpha=.15)
    ct = ax[i].tricontour(x,y, Zmean_nSSW[mask], levels=levels, cmap='seismic',linewidths=1.5)
    gl = ax[i].gridlines(draw_labels=False, color='mediumblue', alpha=.8)
    gl.ylocator = mticker.FixedLocator([60])
    gl.xlines = False
    ax[i].clabel(ct, inline=True, fontsize=6)
...
cb.set_label('% (out of {} tests)'.format(int(num_of_tests)))
plt.tight_layout()
plt.savefig('figures/CNN_nSSW_errorplot_Z.png', dpi=250)
